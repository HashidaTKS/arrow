...
{
    public class ArrowMemoryMappedFileReaderTests
    {
        [Fact]
        public void Ctor_LeaveOpenDefault_StreamClosedOnDispose()
        {
            var stream = new MemoryStream();
            new ArrowMemoryMappedFileReader(stream).Dispose();
        ...
        public async Task Ctor_MemoryPool_AllocatesFromPool()
        {
            ...
            using (MemoryMappedFile mmf = MemoryMappedFile.CreateNew("testmap", 10000))
            {
                using (MemoryMappedViewStream stream = mmf.CreateViewStream())
                {
                    ...
                    ArrowMemoryMappedFileReader reader = new ArrowMemoryMappedFileReader(stream);
        ...
        [Fact]
        public async Task Ctor_MemoryPool_AllocatesFromPool2()
        {
 
            //temporary
            using (MemoryMappedFile mmf = MemoryMappedFile.CreateNew("testmap", 6690))
            {
                using (MemoryMappedViewStream stream = mmf.CreateViewStream())
                {
                    RecordBatch originalBatch = TestData.CreateSampleRecordBatch(length: 100);
                    ...
                    ArrowFileReader reader = new ArrowFileReader(stream, memoryPool, leaveOpen: true);
